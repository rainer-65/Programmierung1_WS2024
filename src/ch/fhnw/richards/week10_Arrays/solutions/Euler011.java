...
public class Euler011 {
	...
	public static void main(String[] args) {
		...
		for (int row = 0; row < inputs.length; row++) {
			for (int col = 0; col < inputs[0].length-3; col++) {
				int product = 1;
				for (int i = col; i <= col+3; i++) {
					product *= inputs[row][i];
				}
		...
		for (int col = 0; col < inputs[0].length; col++) {
			for (int row = 0; row < inputs.length-3; row++) {
				int product = 1;
				for (int i = row; i <= row+3; i++) {
					product *= inputs[i][col];
				}
				if (product > maxValue) maxValue = product;
			}
		}
		...
		for (int row = 0; row < inputs.length-3; row++) {
			for (int col = 3; col < inputs[0].length; col++) {
				int product = 1;
				for (int i = 0; i <= 3; i++) {
					product *= inputs[row+i][col-i];
				}
